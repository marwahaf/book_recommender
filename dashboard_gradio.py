...
final_data = os.path.join(directory, "data_results/books_with_emotions.csv")
books = pd.read_csv(final_data)

books["large_thumbnail"] = books["thumbnail"] + "&fife=w800"
books["large_thumbnail"] = np.where(
    books["large_thumbnail"].isna(),
    "recommender_project/data_results/cover_unavailable.jpg",
    books["large_thumbnail"],
)

text_path = os.path.join(directory, "data_results/tagged_description.txt")
raw_doc = TextLoader(text_path).load()
text_splitter = CharacterTextSplitter(chunk_size=0, chunk_overlap=0, separator="\n")
documents = text_splitter.split_documents(raw_doc)

# db_books = Chroma.from_documents(documents,OpenAIEmbeddings()) #PAYANT
# Replace OpenAIEmbeddings by a free local model
embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")
db_books = Chroma.from_documents(documents, embeddings)


def retrieve_semantic_recommandation(
    query,
    category,
    tone,
    ...
    if category != "All":
        book_recs = books_recs[books_recs["simple_categories"] == category].head(
            final_top_k
        )
    else:
